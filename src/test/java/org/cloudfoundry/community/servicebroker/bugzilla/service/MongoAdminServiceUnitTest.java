...
public class MongoAdminServiceUnitTest {
	 ...
	 * = new MongoAdminService(client); }
	 * 
	 * @Test public void uriConstructedSuccessfullyWithOneServerAddress() {
	 ...
	 * 
	 * assertEquals(expected, service.getConnectionString("database",
	 * "username", "password")); }
	 * 
	 * @Test public void uriConstructedSuccessfullyWithMultipleServerAddresses()
	 * { String expected =
	 * "mongodb://username:password@server1:1,server2:2/database";
	 * 
	 * List<ServerAddress> addresses = new ArrayList<ServerAddress>();
	 * addresses.add(server1); addresses.add(server2);
	 * 
	 * when(server1.getHost()).thenReturn("server1");
	 * when(server1.getPort()).thenReturn(1);
